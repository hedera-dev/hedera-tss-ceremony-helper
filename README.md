...
> **Ceremony window: April 8 – May 12, 2026.** Your node must stay online for the entire window.
...
- **Node ID** — A unique integer identifier for your node, **assigned and provided to you by the Hedera team**.
- **Node private key and public certificate** — Placed in `./keys/`. You can generate them with the
...
- **Online access** — Your node must be able to reach the Internet for the entire ceremony window.
  All nodes communicate through a shared GCP Cloud Storage bucket — there is no offline mode.
...
The ceremony uses GCP Cloud Storage as the communication channel between all participating nodes.
...
Export the GCP credentials and your node ID in your shell before running the scripts:
...
Copy or generate your node key and certificate into the `keys/` directory at the root of this
repository. The directory is mounted into the container as read-only at
`/app/keys/`.

The key and the certificate MUST use a `NODE_ID+1` index for the name of the files. For example, if your `NODE_ID` is `1000000001`, you should create two files named `s-private-node1000000002.pem` and `s-public-node1000000002.pem`.

Example of the final structure of the `keys/` directory if your `NODE_ID` is `1000000001`:

```text
keys/
  s-private-node1000000002.pem    # private key for node ID 1000000001
  s-public-node1000000002.pem     # corresponding certificate for node ID 1000000001
```
